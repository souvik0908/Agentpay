...
async function toKernelPluginManager(client, { sudo, regular, hook, pluginEnableSignature, validatorInitData, action, validAfter = 0, validUntil = 0, entryPoint, kernelVersion, chainId, isPreInstalled = false }) {
    ...
    const isPluginEnabled = async (accountAddress, selector, regularValidator) => {
        const isExternalRegularValidator = !!regularValidator;
        const regular_ = regularValidator ?? regular;
        if (isPreInstalled && !isExternalRegularValidator)
            return true;
        if (!action) {
            throw new Error("Action data must be set");
        }
        ...
        const isEnabled = (await regular_.isEnabled(accountAddress, action.selector)) ||
            (await (0, isPluginInitialized_js_1.isPluginInitialized)(client, accountAddress, regular_.address));
        if (isEnabled && !isExternalRegularValidator) {
            pluginEnabled = true;
        }
        return isEnabled;
    };
    const getPluginEnableSignature = async (accountAddress, regularValidator) => {
        const isExternalRegularValidator = !!regularValidator;
        const regular_ = regularValidator ?? regular;
        if (!action) {
            throw new Error("Action data must be set");
    ...
    const getPluginsEnableTypedData = async (accountAddress, regularValidator) => {
        const regular_ = regularValidator ?? regular;
        if (!action) {
            throw new Error("Action data must be set");
        }
